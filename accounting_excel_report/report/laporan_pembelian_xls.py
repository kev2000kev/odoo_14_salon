...
class LaporanPembelianXlsx(models.AbstractModel):
    _name = 'report.accounting_excel_report.report_laporan_pembelian_xls'
    ...
    def generate_xlsx_report(self, workbook, data, stock):
        ...
        title = workbook.add_format({'bold': True, 'align': 'center', 'border': 1, 'num_format': '#,##0'})
        format_data = workbook.add_format({'border': 1, 'num_format': '#,##0'})
        ...
        sheet.merge_range(1, 0, 1, 9, 'Laporan Penjualan', heading2)
        sheet.merge_range(2, 0, 2, 9, 'Periode : ' + date_to_tanggal(str(data['start'])) + ' - ' + date_to_tanggal(str(data['end'])), heading3)


        sheet.write(6, 0, 'Referensi Order', title)
        sheet.write(6, 1, 'Tanggal', title)
        sheet.write(6, 2, 'Vendor', title)
        sheet.write(6, 3, 'Category', title)
        sheet.write(6, 4, 'Product Name', title)
        sheet.write(6, 5, 'Desc', title)
        sheet.write(6, 6, 'Qty', title)
        sheet.write(6, 7, 'UoM', title)
        sheet.write(6, 8, 'Harga', title)
        sheet.write(6, 9, 'Total', title)

        row = 6
        total = 0
        for i in data['data_pembelian']:
            row += 1
        ...
        sheet.write(row, 8, 'Total', title)
        sheet.write(row, 9, total, title)
