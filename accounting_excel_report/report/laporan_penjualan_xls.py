...
class LaporanPejualanXlsx(models.AbstractModel):
    _name = 'report.accounting_excel_report.report_laporan_penjualan_xls'
    ...
    def generate_xlsx_report(self, workbook, data, stock):
        ...
        sheet.merge_range(0, 0, 0, 10, 'THE PARLOUR HAIRDRESSING', heading1)
        sheet.merge_range(1, 0, 1, 10, 'Laporan Penjualan', heading2)
        sheet.merge_range(2, 0, 2, 10, 'Periode : ' + date_to_tanggal(str(data['start'])) + ' - ' + date_to_tanggal(str(data['end'])), heading3)
        sheet.merge_range(3, 0, 3, 10, 'type : ' + str(data['type']), heading3)
        sheet.merge_range(4, 0, 4, 10, 'Employee : ' + str(data['emp']), heading3)
        ...
        sheet.write(6, 6, 'Customer', title)
        sheet.write(6, 7, 'Qty', title)
        sheet.write(6, 8, 'UOM', title)
        sheet.write(6, 9, 'DPP', title)
        sheet.write(6, 10, 'Amount', title)
        ...
        for i in data['data_penjualan']:
            ...
            sheet.write(row, 10, i[10], format_data)
